...
export function chordLength(radius: number, angleInDegrees: number): number {
  ...
  return 2 * radius * Math.sin(radian/2); // Placeholder - replace with your implementation
}
...
export function distance(p1: Point, p2: Point): number {
  ...
  return Math.sqrt((p1.x - p2.x)** + (p1.y - p2.y)*(p1.y - p2.y)); // Placeholder
}
...
export function findPath(turtle: Turtle, points: Point[]): string[] {
  ...
  for (let i = 1; i < points.length; i++) {
    ...
    const distanceToNext = distance(currentPoint, nextPoint);

    instructions.push(`turn ${angle}`);
    instructions.push(`forward ${distanceToNext}`);
